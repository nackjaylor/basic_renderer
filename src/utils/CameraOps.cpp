...
PointArray CameraOps::remove_distortion(const PointArray& distorted_points) const {
    ...
    while (iter > 0) {
        estimated = (distorted_points.colwise().hnormalized()-second_term).array().colwise()/first_term.array();
...
HPointArray CameraOps::homogenise_point(const PointArray& points) const {
    ...
    HPointArray homogenised = points.colwise().homogeneous();
...
PointArray CameraOps::homogenise_pixel(const PixelArray& pixels) const {
    PointArray homogenised = pixels.colwise().homogeneous();
    return homogenised;
}
